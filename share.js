 ...
 * ■ 配信ドメインを増やすときの手順
 *
 * MITO（mito.bblab.org）と同じキーを使っている。Google Cloud コンソールの
 ...
 * ■ 公開されることについて
 *
 * キーは配信ファイルに書かれる以上、閲覧者からは必ず見える。秘匿は成立しない。
 * GitHub のシークレットスキャンが警告を出すが、この設計では想定内。
 * 失効させても新しいキーが同じように公開されるだけなので、対処にならない。
 *
 * このキーが守っているのは、次の2つの制限である。
 *
 *   - APIの制限（Drive API のみ）… 他のGoogle APIには使えない
 *   - ウェブサイトの制限（リファラー）… 登録外のサイトからの利用と、
 *     リファラーを持たない直接のリクエストを 403 で弾く
 *
 * ただしリファラーは送信側が自由に名乗れるため、狙って偽装する相手は防げない。
 * この制限が防ぐのは他サイトへの貼り付けなど「ついでの流用」であって、
 * 悪用そのものではない。安全の根拠は、偽装されても読めるものが
 * 「リンクを知っている全員」に公開済みのファイルに限られる点にある。
 *
 * したがって最大の実害は割り当ての消費（叩かれると straw と MITO の
 * ドライブ読み込みが止まる）。制限をかけないまま公開しないこと。
 *
 * ■ 共有する側がやること
 *
